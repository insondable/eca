...
def createm3uEntry(file, tvg_id, tvg_logo, group_title, tvg_name, m3u8_link, extra_flags, user_agent , tvg_name2):
    if (not m3u8_link):
    ...
    if tvg_name == tvg_name2:
        global n 
        tvg_name = tvg_name2 + ' #' + str(n)
        n = n+1
    else :     
        tvg_name = tvg_name
        n = 2
   
    string_format = f'#EXTINF: -1 tvg-id="{tvg_name}" tvg-logo="{tvg_logo}" group-title="{group_title}" tvg-name="{tvg_name}",{tvg_name}'
...
if "--extra-flags" in sys.argv:
    extra_flags = True
w3u_filenames = glob.glob("wise/*.w3u")
